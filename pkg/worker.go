...
func (w *Worker[T]) Run(ctx context.Context) error {
    next := FilterFunc[T](func(ctx context.Context, event Event[T]) error {
        return w.output.OnWrite(ctx, []Event[T]{event})
    })
    sink := EventSink[T](func(event Event[T]) error {
        return makeFilterChain(next, w.filters)(ctx, event)
    })
